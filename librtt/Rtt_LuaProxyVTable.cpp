...
// Proxy's delegate or an ancestor must match expected
	...
	*/
	...
	// ----------begin---------------------LuaOutlineTextObjectProxyVTable---------------------------------------------

	//从SpineUtil.h拷贝过来的.
	static unsigned constexpr const_hash(char const* input) { return *input ? static_cast<unsigned int>(*input) + 33 + const_hash(input + 1) : 131; }
	static constexpr unsigned int  Str2Hash(const char* str) { return const_hash(str); }


	constexpr  char* outlinetext_fillVertColor = "fillVertColor";
	constexpr  char* outlinetext_outlineColor = "outlineColor";
	constexpr  char* outlinetext_outlineWidth = "outlineWidth";
	constexpr  char* outlinetext_nineArgs = "nineArgs";
	constexpr char* outlinetext_meshIsDirty = "meshIsDirty";
	const LuaProxyVTable& OutlineTextObject::ProxyVTable() const
	{
		return LuaOutlineTextObjectProxyVTable::Constant();
	}

	const LuaOutlineTextObjectProxyVTable& LuaOutlineTextObjectProxyVTable::Constant()
	{
		static const LuaOutlineTextObjectProxyVTable kVTable;
		return kVTable;
	}

	const LuaProxyVTable& LuaOutlineTextObjectProxyVTable::Parent() const
	{
		return Super::Constant();
	}

	int LuaOutlineTextObjectProxyVTable::ValueForKey(lua_State* L, const MLuaProxyable& object, const char key[], bool overrideRestriction /*= false*/) const
	{
		if (!key) { return 0; }

		int result = 1;

		int apiNameHash = Str2Hash(key);

		// TextObject* o = (TextObject*)LuaProxy::GetProxyableObject( L, 1 );
		const OutlineTextObject& o = static_cast<const OutlineTextObject&>(object);
		Rtt_WARN_SIM_PROXY_TYPE(L, 1, OutlineTextObject);
		switch (apiNameHash)
		{
		case Str2Hash(outlinetext_outlineColor): { OutlineTextObject::BindingProperty_Get_OutlineColor(L, object); result = 1; }break;
		case Str2Hash(outlinetext_outlineWidth): { OutlineTextObject::BindingProperty_Get_OutlineWidth(L, object); result = 1; }break;
		case Str2Hash(outlinetext_nineArgs): { OutlineTextObject::BindingProperty_Get_NineArgs(L, object); result = 1; }break;
		case Str2Hash(outlinetext_meshIsDirty): { OutlineTextObject::BindingProperty_Get_MeshIsDirty(L, object); result = 1; }break;
		case Str2Hash(outlinetext_fillVertColor): { OutlineTextObject::BindingProperty_Get_FillVertColor(L, object); result = 1; }break;

		default: {result = Super::ValueForKey(L, object, key, overrideRestriction); }
			   break;
		}
		return result;
	}

	bool LuaOutlineTextObjectProxyVTable::SetValueForKey(lua_State* L, MLuaProxyable& object, const char key[], int valueIndex) const
	{
		if (!key) { return false; }

		OutlineTextObject& o = static_cast<OutlineTextObject&>(object);
		Rtt_WARN_SIM_PROXY_TYPE(L, 1, OutlineTextObject);
		bool result = false;
		int apiNameHash = Str2Hash(key);
		switch (apiNameHash)
		{
		case Str2Hash(outlinetext_fillVertColor): {OutlineTextObject::BindingProperty_Set_FillVertColor(L, object, valueIndex); result = true; }break;
		case Str2Hash(outlinetext_outlineColor): { OutlineTextObject::BindingProperty_Set_OutlineColor(L, object, valueIndex); result = true; }break;
		case Str2Hash(outlinetext_nineArgs): { OutlineTextObject::BindingProperty_Set_NineArgs(L, object, valueIndex); result = true; }break;
		case Str2Hash(outlinetext_outlineWidth): {OutlineTextObject::BindingProperty_Set_OutlineWidth(L, object, valueIndex); result = true; }break;
		default: {result = Super::SetValueForKey(L, object, key, valueIndex); }break;
		}
		return result;
	}

	// ---------------end----------------LuaOutlineTextObjectProxyVTable---------------------------------------------
